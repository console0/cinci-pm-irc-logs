 ...
 *
 * Adjustments made by Online Rewards (to 5.3.1)  7/22/2014
*/
        ...
        .on('touchend.fndtn.reveal click.fndtn.reveal', this.close_targets(), function (e) {
          ...
          if (!self.locked) {
            var settings = S('[' + self.attr_name() + '].open').data(self.attr_name(true) + '-init');
              ...
              //OLR HACK: Gets around console error when close event is triggered twice
              //"Cannot read property 'bg_class' of null"
              
              if(settings) {
                var bg_clicked = S(e.target)[0] === S('.' + settings.bg_class)[0];
...
;(function ($, window, document, undefined) {
  'use strict';
    ...
    //olr: modified to allow "querystring" appended to end of hashes
    //when deep linking option is enabled 
  ...
  Foundation.libs.tab = {
    ...
    handle_location_hash_change : function () {
      ...
      S('[' + this.attr_name() + ']', this.scope).each(function () {
        ...
        if (settings.deep_linking) {
          ...
          var hash = self.scope.location.hash.split('?')[0];
    ...
    toggle_active_tab: function (tab, location_hash) {
      var S = this.S,
          ...
          target_hash = '#' + anchor.attr('href').split('#')[1].split('?')[0],
          qs = location.hash.indexOf('?') == -1 ? "" : '?' + location.hash.split('?')[1],
      ...
      if (settings.deep_linking) {
        ...
        if (location_hash != undefined) {
          window.location.hash = location_hash + qs;
        } else {
          window.location.hash = target_hash + qs;
        }
