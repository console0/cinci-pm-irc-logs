...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:exsl="http://exslt.org/common" version="1.0">
    ...
    <xsl:template name="field-wrapper">
        ...
        <div>
            ...
            <xsl:if test="exsl:node-set($attrs)/attrs/@description">
            
                <div>
                    <xsl:value-of select="exsl:node-set($attrs)/attrs/@description"/>
                </div>
            </xsl:if>
        </div>
    </xsl:template>
    ...
    <xsl:template name="input">
        ...
        <xsl:call-template name="field-wrapper">
            ...
            <xsl:with-param name="attrs"><xsl:copy-of select="attrs"/></xsl:with-param>
    ...
    <!-- Text widget -->
    <xsl:template match="field[@widget='autocomplete']" mode="auto">
        <xsl:param name="prefix"/>
    ...
    <!-- AJAX autocomplet/select list -->
    <xsl:template match="field" mode="autocomplete">
        <xsl:param name="prefix"/>
        <xsl:call-template name="field-wrapper">
            <xsl:with-param name="prefix" select="$prefix"/>
            <xsl:with-param name="attrs"><xsl:copy-of select="attrs"/></xsl:with-param>
            <xsl:with-param name="field">
                ...
                <select id="{@id}" name="{@name}" data-form="autocomplete">
                    <xsl:for-each select="attrs/@*">
                        <xsl:attribute name="{name()}">
                            <xsl:value-of select="."/>
                        </xsl:attribute>
                ...
                <input type="text" id="{@id}_ac" name="{@name}_ac" data-widget="text" value="{@value}">
                    <xsl:for-each select="attrs/@*">
                        <xsl:attribute name="{name()}">
                            <xsl:value-of select="."/>
                        </xsl:attribute>
